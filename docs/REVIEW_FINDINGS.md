...
Reviewed on 2026-07-20 against the local `docsera` and `docsera-mobile` references and the current starter implementation. Status layer re-verified on 2026-08-17 after roadmap Phase 3 (backend foundation hardening).
...
The prototype proves the core integration loop — Firebase identity, a Firestore user, a server-side AI call, and an Expo client — and the backend suite is green (40 tests, 0 failures). After Phase 3 the foundation is materially stronger: fail-closed cloud configuration with explicit `local` mocks, a versioned OpenAPI contract with one error envelope, a supported dependency baseline (Spring Boot 4.1, Spring AI 2.0), bounded AI calls (`429`+`Retry-After`, input caps, timeout), an SBOM-producing build, and repeatable DEV/PROD infrastructure code. What remains is largely external or later-phase: real repository separation with tags/remotes, CI-gated and digest-only deployment, mobile delivery determinism, and a handful of documented caveats.
...
8. **The mobile toolchain is not deterministic locally.** [Open — Phase 4] Unchanged: `expo lint` still fails on the `unrs-resolver` native binding; direct ESLint and TypeScript pass.
...
- [Partially — Phase 3] API contract tests: backend `OpenApiContractTest` done; mobile contract validation (`npm run validate:contract`) done; mobile integration fixtures remain (Phase 4).
...
- **C9 — Mobile advisory count rose at re-check.** `npm audit` is now **78 vulnerabilities (10 moderate, 68 high)** vs the 14 moderate at review — chains via `brace-expansion`/`minimatch` in dev/build tooling (Expo, Metro, ESLint, typescript-estree), mostly non-runtime; triage during the mobile dependency-baseline pass.
- **C10 — Mobile `expo lint` non-determinism and single-arch Node.** Direct ESLint/tsc pass; the Expo wrapper still fails on the optional `unrs-resolver` binding (Phase 4; pin one supported Node version/architecture in scripts and CI).

## Verification performed (updated 2026-08-17)
...
| Mobile `validate:contract` | Pass (pinned backend contract v1) |
| Mobile `npx tsc --noEmit` | Pass |
| Mobile `eslint . --no-cache` | Pass |
| Mobile `npm run lint` (`expo lint`) | Fail — unchanged (import resolver / native binding in dual-architecture Node setup) |
| Mobile `npm audit` | 78 vulnerabilities (10 moderate, 68 high) — see C9 |
...
## Recommended decision set (status at Phase 3)
...
- DEV preview builds separate from store release-candidate builds — Open (mobile)
