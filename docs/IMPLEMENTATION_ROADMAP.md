...
Checkbox legend: `[x]` complete · `[~]` partially complete (note explains the remainder) · `[ ]` not started. Status updated 2026-08-17 after Phase 3; workflow names/counts refreshed 2026-09-05.
...
- [x] Approve repository names, GitHub organization, and history strategy — history strategy settled (per-repository `git subtree split` preserving full history); repos pushed under `patrikbego` (2026-08-19–23)
...
- [x] Move each workflow to its owning repository root — backend: `ci.yml`, `verify.yml`, `deploy-dev.yml`, `promote-prod.yml`, `terraform-plan.yml`; mobile: `ci.yml`, `build-preview.yml`, `build-release.yml`, `submit-release.yml`, `deploy-web.yml`, `web-e2e.yml`
...
> Status note: extraction and local verification are complete; the two repositories are pushed to `patrikbego/starter-backend` and `patrikbego/starter-mobile` (main, with remotes), and CI runs on the platform. Branch protection, Dependabot alerts, secret scanning, and the protected `production` environment are settings to enable per README ("Repository settings (enable after first push)").
...
- [x] Reinstall dependencies from a clean lockfile state and make lint deterministic (`npm ci` + `.nvmrc` Node 22 + direct ESLint in CI; 32 Jest tests green from clean install; 105 tests by 2026-08-29, 201 by 2026-09-05 — see the caveat register)
